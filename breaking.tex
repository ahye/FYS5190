...
\begin{equation}
\str{M^2} \equiv \sum_s (-1)^{2s}(2s+1)\,\tr{M_s^2},
\end{equation}
where $M$ is the complete mass matrix of the Lagrangian, $s$ is the spin of particles and $M_s$ is the mass matrix of all spin-$s$ particles.}
...
\begin{Answer}
The Fayet-Iliopolous term is given by
\begin{equation*}
    \mathcal{L} \supset \int d^4\theta kV = kD.
\end{equation*}
Including the other terms containing the $D$-field, that were found in Exercise~\ref{chap:lagrangian}.\ref{ex:SQED}, we find
\begin{equation*}
    \mathcal{L} \supset 2D^2 + e(|A_L|^2-|A_E|^2) + kD,
\end{equation*}
which gives the equation of motion for $D$:
\begin{align*}
    \frac{\partial \mathcal{L}}{\partial D} &= 4D + e(|A_L|^2-|A_E|^2) + k = 0
    \\
    D &= -\frac{1}{4} \big[ e(|A_L|^2-|A_E|^2) + k \bigr],
\end{align*}
and it follows that the square of $D$ is
\begin{equation*}
    D^2 = \frac{1}{16} \bigl[
        e^2 (|A_L|^2-|A_E|^2)^2 + 2ek(|A_L|^2-|A_E|^2) + k^2
    \bigr].
\end{equation*}
For this to be zero the first term requires $(|A_L|^2-|A_E|^2)=0$, but this gives $D^2 = \frac{k^2}{16} \neq 0$, thus we have that $D^2 > 0$, and consequently for the scalar potential:
\begin{equation*}
    V(A_{L,E},A_{L,E}^*) = |F_L|^2 + |F_E|^2 + D^2 > 0,
\end{equation*}
which means that supersymmetry is spontaneously broken.
\end{Answer}
%%%
...
\begin{Answer}
From Exercise~\ref{ex:SQED_FI} we have
\begin{equation*}
    D = -\frac{1}{4} \bigl[ e(|A_L|^2-|A_E|^2) + k \bigr],
\end{equation*}
and from this we can find all the Lagrangian terms containing the $D$-field:
\begin{align*}
    \mathcal{L} &\supset e(|A_L|^2-|A_E|^2)D + 2D^2 + kD
    \\
    &= -\frac{e}{4} (|A_L|^2-|A_E|^2) \bigl[ e(|A_L|^2-|A_E|^2)+k \bigr]
    \\ &\qquad + \frac{1}{8} \bigl[ e(|A_L|^2-|A_E|^2)+k \bigr]^2 - \frac{k}{4} \bigl[ e(|A_L|^2-|A_E|^2)+k \bigr]
    \\
    &= -\frac{e^2}{8} (|A_L|^2-|A_E|^2)^2 - \frac{ek}{4} (|A_L|^2-|A_E|^2) - \frac{k^2}{8}.
\end{align*}
The second term in the above equation gives two new mass contributions to the scalar component fields $A_L$ and $A_E$. The mass terms in the Lagrangian are then
\begin{align*}
    \mathcal{L}_{\mathrm{mass}} = -\bigg( |m|^2 + \frac{ek}{4} \bigg) |A_L|^2 - \bigg( |m|^2 - \frac{ek}{4} \bigg) |A_E|^2 -m\bar\Psi\Psi ,
\end{align*}
where we have used that the Dirac spinor for the electron can be written $\Psi=(\psi_L,\bar\psi_E)^T$ and the positron as  $\bar\Psi=(\psi_E,\bar\psi_L)^T$, so that the mass term is
\[
m\psi_L\psi_E + m^*\bar\psi_L\bar\psi_E = m\bar\Psi\Psi 
\]
This gives the supertrace
\begin{align*}
    \str{M^2} &= \sum_s (-1)^{2s}(2s+1) \Tr[M_s^2]
    \\
    &= \Tr\Bigg[ \begin{matrix}
        |m|^2 + \frac{ek}{4} & 0 \\
        0 & |m|^2 - \frac{ek}{4}
    \end{matrix} \Bigg] - 2 |m|^2
    \\
    &= |m|^2 + \frac{ek}{4} + |m|^2 - \frac{ek}{4} - 2 |m|^2
    \\
    &= 0,
\end{align*}
as expected.
