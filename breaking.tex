...
\end{align*}
This gives two new mass contributions to the scalar component fields $A_L$ and $A_E$. The mass terms in the Lagrangian are then
\begin{align*}
    \mathcal{L}_{\mathrm{mass}} = -\bigg( |m|^2 + \frac{ek}{4} \bigg) |A_L|^2 - \bigg( |m|^2 - \frac{ek}{4} \bigg) |A_E|^2 + m\psi_L\psi_E + m^*\bar\psi_L\bar\psi_E,
\end{align*}
giving the supertrace
\begin{align*}
    \operatorname{STr}(M^2) &= \sum_s (-1)^{2s}(2s+1) \operatorname{Tr}[M_s^2]
    \\
    &= \operatorname{Tr}\Bigg[ \begin{pmatrix}
        |m|^2 + \frac{ek}{4} & 0 \\
        0 & |m|^2 - \frac{ek}{4}
    \end{pmatrix} \Bigg] - 2 |m|^2
    \\
