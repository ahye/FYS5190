...
\end{equation}
As a consequence all three ratios $M_i/g_i^2$ are scale independent at one loop. To see this let $R_i=M_i/g_i^2$, then
\begin{equation}
...
\end{equation}
In other words, $R_i$ does not change with scale $t$.
...
\end{equation}
which, inserting values for the fine structure constant $\alpha$, the strong coupling $\alpha_s$, and the Weinberg angle $\sin^2\theta_W$, numerically predicts
