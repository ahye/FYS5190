...
These conventions lead to the wanted Standard Model gaugeinteractions for the component fields and the familiar relations after electroweak symmetry breaking, $e = g\sin\theta_W = g'\cos\theta_W$, where $e$ is the elementary electric charge (in natural units).
...
a [particle=$A$] -- [boson] b,
b -- [boson] c [particle=$V$],
b -- [boson] d [particle=$V$],
};
%
\diagram [xshift=6cm, small, horizontal=a to t1] {
a [particle=$A$] -- [boson] t1 -- [fermion] t2 -- [fermion, edge label=$f$] t3 -- [fermion] t1,
t2 -- [boson] p1 [particle=$V$],
t3 -- [boson] p2 [particle=$V$],
p1 -- [opacity=0] p2,
};
\end{feynman}
\end{tikzpicture}
\caption{The tree level coupling between three gauge bosons with vector $V$ and axial vector $A$ couplings (left), and the one-loop fermion contribution to the same process (right).}  
\label{fig:anomaly}
\end{figure}
%%%
...
\item The {\bf lightest supersymmetric particle} (LSP) is absolutely stable, and every other sparticle must decay down to the LSP (possibly in multiple steps, a so-called cascade).
\end{enumerate}
...
\end{equation}
which describes the behaviour of a Lagrangian parameter $\lambda$ as a function of the energy scale $\mu$ away from the value where it was defined, often denoted $\mu_0$. The {\bf anomalous dimension} of the field
\begin{equation}
\boldsymbol\gamma =  \mu\frac{\partial}{\partial\mu}\ln Z^\half_\Phi,
\end{equation} 
describes the scaling of the fields and the factor $n_\Phi$ gives the number of external fields (of a given kind) in the effective action. The RGE balances the different contributions to sum to zero.
...
For the soft-breaking parameters $M_i$ in (\ref{eq:gensoftterms}) we similarly have the one-loop  $\beta$-functions
\begin{equation}
...
%%%%%
\subsection{$\beta$-functions in the MSSM}
...
Some  examples of other $\beta$-functions in the MSSM:
\begin{itemize}
\item For the supersymmetry parameter $\mu$
\begin{equation}
\beta_{\mu}\equiv \frac{d}{dt} \mu = \frac{1}{16\pi^2}\mu [3|y_t|^2+3|y_b|^2+|y_\tau|^2-3g_2^2-\frac{3}{5}g_1^2],
\label{eq:mu_beta}
\end{equation}
where $y_t$, $y_b$ and $y_\tau$, are the top and bottom quark, and tau lepton Yukawa couplings. Other Yukawa couplings also contribute to one-loop order, but due to their small size they can usually be safely ignored. Since the gauge couplings appear on the right-hand side here, we see that the differential equations that determine the running of $\mu$ and the gauge couplings are coupled. It is a general property of the $\beta$-functions that they couple the running of all parameters, which is caused by the (eventual) appearance of all particles as virtual particles in loops when we go to high-enough order.
\item For the soft-breaking parameters $M_i$ in (\ref{eq:gensoftterms}) we get the one-loop $\beta$-functions
\begin{equation}
\beta_{M_i}\equiv \frac{d}{dt}M_i = \frac{1}{8\pi^2}g_i^2M_ib_i,
\label{eq:beta_Mi}
\end{equation}
where $b_i$ is given above.
\item For the Higgs soft masses
\begin{eqnarray}
\beta_{m_{H_u}^2} &\equiv& \frac{dm_{H_u}^2}{dt} = \frac{1}{16\pi^2}(X_t-\frac{6}{5} g_1^2|M_1|^2- 6g_2^2|M_2|^2)  ,\label{eq:beta_Hu2}\\
\beta_{m_{H_d}^2} &\equiv& \frac{dm_{H_d}^2}{dt} = \frac{1}{16\pi^2}( X_b +X_\tau -\frac{6}{5} g_1^2|M_1|^2 - 6g_2^2|M_2|^2) \label{eq:beta_Hd2},
\end{eqnarray}
where we have used the standard abbreviations
\begin{eqnarray*}
X_t &\equiv& 6 |y_t|^2(m_{H_u}^2 + m_{Q_3}^2 + m_{u_3}^2) + 6|a_t|^2,\\
X_b &\equiv& 6 |y_b|^2(m_{H_d}^2 + m_{Q_3}^2 + m_{d_3}^2) + 6|a_b|^2,\\
X_\tau &\equiv& 2 |y_\tau|^2(m_{H_d}^2 + m_{L_3}^2 + m_{e_3}^2) + 2|a_\tau|^2,
\end{eqnarray*}
and where $m_{Q_3} = m_{33}^Q$, $m_{u_3}  = m_{33}^u$, $m_{d_3} = m_{33}^d$, $a_t=a_{33}^u$, $a_b=a_{33}^d$, and $a_\tau=a_{33}^e$,  in a simplification of our previous notation for the soft-term parameters in Sec.~\ref{sec:MSSM_soft_terms}.  
\end{itemize}
...
By running the couplings $\alpha_i^{-1}$ from their values measured at the electroweak scale to high energies it is observed that in the MSSM the coupling constants intersect at a single point, called {\bf gauge coupling unification}, which they do not naturally do in the Standard Model. See Fig.~\ref{fig:unification}, taken from Martin~\cite{Martin:1997ns}. The common assumption made is then that a unified gauge group, {\it e.g.}\ $SU(5)$ or $SO(10)$, is broken at that scale, called the {\bf grand unification theory} scale or GUT-scale, down to the Standard Model gauge group. This scale is $\mu_\text{GUT} \approx 2\cdot 10^{16}$\,GeV, about two orders of magnitude below the Planck scale. 
...
Something funny happens to the gaugino soft-mass parameters $M_i$ if we look at their running. From (\ref{eq:beta_Mi}) we find that all three ratios $M_i/g_i^2$ are scale independent at one loop. To see this let $R_i=M_i/g_i^2$, then
\begin{equation}
\beta_{R_i} \equiv \frac{dR_i}{dt}= \frac{\frac{dM_i }{dt}g_i^2 - M_i \frac{d}{dt} g_i^2}{g_i^4} = \frac{\frac{1}{8\pi^2}g_i^2M_ib_i\cdot g_i^2 - M_i\cdot 2g_i\cdot\frac{1}{16\pi} g_i^3 b_i}{g_i^4} = 0.
\end{equation}
In other words, $R_i$ does not change with scale $t$ at one-loop level.
...
\end{equation}
which, inserting values for the fine structure constant $\alpha$, the strong coupling $\alpha_s$, and the Weinberg angle $\sin^2\theta_W$, evaluated at the electroweak scale,  numerically predicts
\[M_3:M_2:M_1 \simeq 6:2:1.\]
We will return to the implications of this when discussing the gauginos in Sec.~\ref{sec:electroweakinos}.

In Fig.~\ref{fig:MSSMrun}, again taken from Martin~\cite{Martin:1997ns}, we show the running of all the types of soft parameters in the MSSM. We assume unified soft-mass parameters $m_{1/2}$ for the gauginos and $m_0$ for the Higgs and sfermions at the GUT scale. Shown is the gaugino mass parameters $M_i$ (solid black), the Higgs mass parameters $m_{H_{d/u}}^2$ (dot-dashed green), the third generation sfermion soft terms $m_{d_3}$, $m_{Q_3}$, $m_{u_3}$, $m_{L_3}$ and $m_{e_3}$ (dashed blue and red, listed from top to bottom), and the corresponding first and second generation terms $m_{d_1}=m_{d_2}$, $m_{Q_1}=m_{Q_2}$, $m_{u_1}=m_{u_2}$, $m_{L_1}=m_{L_2}$ and $m_{e_1}=m_{e_2}$ (solid blue and red lines).
%%%%%%%%% 
\begin{figure}[h]
\centering
...
\label{sec:rewsb}
%%%%%%%%%%%%%%%%%%%%%%%%%%
In the Standard Model the gauge symmetries prevent mass terms for both vector bosons and fermions. The $W$ and $Z$ bosons, and all the fermions are given mass by spontaneous electroweak symmetry breaking (EWSB), which is induced by the shape of the scalar potential for the Higgs field $H$, which is an $SU(2)_L$ doublet of scalar fields:
\begin{equation}
...
We start by using our $SU(2)_L$ gauge freedom, picking a gauge so that we rotate away any field value for $H_u^+$ at the minimum of the potential.\footnote{Can this always be done? Yes. The action of a member of $SU(2)$ on the two component vector of Higgs fields is 
\[ \left [\begin{matrix} \alpha & -\beta^*  \\ \beta & \alpha^* \end{matrix}\right]  \left( \begin{matrix} H_u^+ \\ H_u^0 \end{matrix}\right),\] 
with $|\alpha|^2+|\beta|^2=1$, where $\alpha$ and $\beta$ can be picked so that $\alpha H_u^+ -\beta^* H_u^0 =0$ by choosing $\alpha=H_u^0/(|H_u^0|^2+ |H_u^+|^2)$ and $\beta^*=H_u^+/(|H_u^0|^2+ |H_u^+|^2)$.} So without loss of generality we can use that $H_u^+ = 0$ at the minimum in what follows. At the minimum we must also have $\partial V/\partial H_u^+= 0$ since it is a minimum. By explicit differentiation of the potential\footnote{Warning! We are here taking the partial differential with respect to a complex number.} 
\begin{eqnarray}
\frac{\partial V}{\partial H_u^+} &=& |\mu|^2 H_u^{+*} +\frac{1}{4} (g^2 + g'{}^2)(|H^0_u|^2 + |H_u^+|^2 - |H^0_d|^2 - |H_d^-|^2) H_u^{+*} \nonumber\\
&& +\half  g^2(H_u^{+*}H_d^0 + H_u^{0*} H^-_d) H_d^{0*} + m_{H_u}^2 H_u^{+*} + bH_d^-,
\end{eqnarray}
we can see that $H_u^+ = 0$ also leads to $H^-_d = 0$ at the minimum. This is good and proper since it will work as a guarantee for our item ii), that $U(1)_{\rm em}$ is a symmetry for the minimum of the potential, since the charged fields then have no vevs and the minimum is then unaffected by $U(1)_{\rm em}$ gauge transformations. 
...
In fact, if we assume that $m_{H_d} = m_{H_u}$ at some scale  $\mu$, for example the GUT scale, then \eqref{eq:higgsbound2} and \eqref{eq:higgsbound1} cannot be simultaneously be satisfied {\it at that scale}. However, to 1-loop the RGE running of these mass parameters were given in (\ref{eq:beta_Hu2}) and (\ref{eq:beta_Hd2}). Because $y_t\gg y_b$, $m_{H_u}^2$ runs much faster with scale than $m_{H_d}^2$. If the parameters start out the same at some high scale, say from some universal supersymmetry breaking effect,  as we go down to the electroweak scale  $m_{H_u}^2$ becomes significantly smaller than  $m_{H_d}^2$, and may become negative, fulfilling the EWSB criteria. For an illustration, see Fig.~\ref{fig:MSSMrun}, where the running of  $\mu^2+m_{H_u}^2$ and  $\mu^2+m_{H_u}^2$ is shown.
...
%%%%%
\subsection{Parameters of the Higgs sector}
To summarise what we have learnt so far: at the minimum of the Higgs potential we know that there exists a gauge choice so that the expectation values of the charged Higgs component fields are zero, $\langle H_u^+\rangle =0$ and $\langle H_d^-\rangle =0$, and we fulfil the remaining conditions for the existence of an extremal point in the neutral Higgs component fields by
\begin{equation}
\frac{\partial V}{\partial H_u^0}=\frac{\partial V}{\partial H_d^0}= 0.
\label{eq:EWSB_condition}
\end{equation}
These conditions require at the minimum
\begin{eqnarray}
\frac{\partial V}{\partial H_d^0} &=&(|\mu|^2  + m_{H_d}^2) H_d^{0*} -\frac{1}{4} (g^2 + g'{}^2)(|H^0_u|^2  - |H^0_d|^2 )H_d^{0^*}   -b H_u^0 =0 , \label{eq:EWSB_condition_Hd0} \\
\frac{\partial V}{\partial H_u^0} &=&(|\mu|^2  + m_{H_u}^2) H_u^{0*} +\frac{1}{4} (g^2 + g'{}^2)(|H^0_u|^2  - |H^0_d|^2 )H_u^{0^*}   -b H_d^0 = 0.\label{eq:EWSB_condition_Hu0}
\end{eqnarray}
...
2b &<& 2|\mu|^2 + m_{H_u}^2 + m_{H_d}^2. \nonumber
\end{eqnarray}
The resulting non-zero expectation values at the minimum for the neutral Higgs component fields are denoted $\langle H_u^0\rangle \equiv v_u$ and $\langle H_d^0\rangle \equiv v_d$.
...
Using the condition for the existence of an extremal point in (\ref{eq:EWSB_condition_Hd0}) and (\ref{eq:EWSB_condition_Hu0}), we then have the relationships
\begin{eqnarray}
|\mu|^2  + m_{H_d}^2 -\half M_Z^2\cos(2\beta)  -b \tan\beta&=& 0 , \label{eq:EWSB_condition_Hd0_vevs} \\
|\mu|^2  + m_{H_u}^2 +\half M_Z^2\cos(2\beta) -b\cot\beta  &=& 0.\label{eq:EWSB_condition_Hu0_vevs}
\end{eqnarray}
Using these, the two non-SM parameters $b$ and $|\mu|$ can be eliminated as free parameters from the model, however, not the sign of $\mu$. Alternatively, we can choose to eliminate $m_{H_u}^2$ and $m_{H_d}^2$. You can look at this as giving away the freedom of these parameters to the vevs, and then fixing one vev by the electroweak constraint, and using $\tan\beta$ for the other.
...
Another way to look at this is to solve (\ref{eq:EWSB_condition_Hd0_vevs}) and  (\ref{eq:EWSB_condition_Hu0_vevs})  for the well-measured $Z$-boson mass and $\sin(2\beta)$:
\begin{eqnarray*}
\sin(2\beta) &=& \frac{2b}{m_{H_d}^2+m_{H_u}^2+2|\mu|^2}, \\
M_Z^2 &=& \frac{|m_{H_d}^2-m_{H_u}^2|}{\sqrt{1-\sin^2(2\beta)}}-m_{H_u}^2-m_{H_d}^2-2|\mu|^2.
\end{eqnarray*}
Now, unless all the soft-masses $b$, $m_{H_u}^2$ and $m_{H_d}^2$ and the supersymmetric parameter $\mu$ is close to the electroweak scale there must be a great fine-tuning of parameters to get the correct $Z$-boson mass, and $\mu$ can certainly not be very much larger than the other three parameters.


%%%%%%%%%%%%%%%%%
\section{Higgs boson properties}
%%%%%%%%%%%%%%%%%
Of the eight d.o.f.\ in the scalar potential for the Higgs component fields three are Goldstone boson (real scalar fields) $G$ and $G^\pm$ that get eaten by $Z$ and $W^\pm$ to give them masses. The remaining five d.o.f.\ form two neutral (real) scalars $h$, $H$, two charged scalars $H^\pm$ and one neutral pseudo-scalar (CP-odd) $A$.\footnote{In addition to the scalars, we know that the Higgs supermultiplets contain four fermions, $\tilde{H}^0_u$, $\tilde{H}^0_d$, $\tilde{H}^+_u$ and $\tilde{H}^-_d$ (higgsinos). We will see later that these mix with the fermion partners of the gauge bosons (gauginos).} 

We can decompose the complex Higgs fields in these fields by writing them as
\begin{eqnarray*}
\left( \begin{matrix} H_u^0 \\ H_d^0 \end{matrix}\right) &=& \left( \begin{matrix} v_u \\ v_d \end{matrix}\right) +\frac{1}{\sqrt{2}} R_\alpha \left( \begin{matrix} h \\ H \end{matrix}\right)  +\frac{i}{\sqrt{2}} R_\beta \left( \begin{matrix} G \\ A \end{matrix}\right),  \\
\left( \begin{matrix} H_u^+ \\ H_d^{-*} \end{matrix}\right) &=& R_\beta \left( \begin{matrix} G^+ \\ H^+ \end{matrix}\right),
\end{eqnarray*}
where the (orthogonal) mixing matrices are parameterised as
\begin{equation}
R_\alpha = \left( \begin{matrix} \cos\alpha &  \sin\alpha \\  -\sin\alpha & \cos\alpha \end{matrix}\right), \quad  R_\beta = \left( \begin{matrix} \sin\beta &  \cos\beta \\  -\cos\beta & \sin\beta \end{matrix}\right),
\end{equation}
Inserting these in to the quadratic terms in (\ref{eq:MSSMscalarpot})---the mass terms---we get the Lagrange terms
\begin{equation}
V=\half m_h^2 h^2 +\half m_H^2 H^2 +\half m_G^2 G^2 +\half m_A^2 A^2 + m^2_{G^\pm} |G^\pm|^2 + m^2_{H^\pm}
 |H^+|^2,
 \end{equation}
where the tree-level masses are
\begin{eqnarray}
m_G^2 &=& 0, \label{eq:HiggsmassesG} \\
m_{G^\pm}^2 &=& 0, \\
...
m_{H^\pm}^2 &=& m_A^2 + m_W^2, \label{eq:HiggsmassesH}
\end{eqnarray}
with 
\begin{equation}
...
where $m_0$ is the Lagrangian bare mass, $\Sigma$ is the self-energy, and the pole mass is the solution $\slashed p=m$ to the equation $\slashed{p}-m_0-\Sigma(\slashed p) =0$.
...
The mass matrix can now be diagonalised to find the $\tilde\chi^0_i$ masses. If $N$ is a unitary diagonalisation matrix for $M_{\tilde\chi}$, we can write\footnote{A symmetric matrix $M$ is always unitary diagonalisable, even if complex. For real symmetric matrices the diagonalisation can be carried out with an orthogonal matrix $N$ (which is a special case of unitary), giving the diagonal matrix $D=NMN^T$ with real, but possibly negative, entries. For complex matrices this can be done by so-called {\bf Takagi factorisation} using a unitary matrix $N$ so that $D=NMN^T$ is a diagonal matrix with real non-negative entries.}
...
\begin{Exercise}[]
Make the insertion into the scalar potential to show that we get the Higgs masses in (\ref{eq:HiggsmassesG})--(\ref{eq:HiggsmassesH}), and find the requirement on the mixing parameter $\alpha$.
\end{Exercise}
