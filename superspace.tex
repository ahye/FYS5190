...
\[\partial_A \theta^B \equiv \frac{\partial}{\partial \theta^A} \theta^B \equiv \delta_A{}^B,\quad  \bar\partial_{\dot{A}} \bar\theta^{\dot B} \equiv \frac{\partial}{\partial \bar\theta^{\dot A}} \bar\theta^{\dot B} \equiv \delta_{\dot A}{}^{\dot B}\]
with a product rule due to the anti-commuting nature of the superspace coordinates
\begin{eqnarray}
\partial_A(\theta_1^{B_1} \theta_2^{B_2}\theta_3^{B_3}\ldots\theta_n^{B_n} ) &\equiv& (\partial_A\theta_1^{B_1}) \theta_2^{B_2}\theta_3^{B_3}\ldots\theta_n^{B_n}\nonumber\\
&&- \theta_1^{B_1}(\partial_A\theta_2^{B_2})\theta_3^{B_3}\ldots\theta_n^{B_n}\nonumber\\
 && +\ldots+(-1)^{n-1}\theta_1^{B_1}\theta_2^{B_2}\ldots(\partial_A\theta_n^{B_n}),
\label{eq:superspace_diff_product_rule}
 \end{eqnarray}
and similarly for $\bar\partial$.}
This implies that the differential operator $\partial_A$ is itself a Grassmann number and anti-commutes 
\[\{\partial_A, \partial_B\} = \{\partial_A, \bar\partial_{\dot{B}}\} = \{ \bar\partial_{\dot{A}}, \partial_B\} = \{\bar\partial_{\dot{A}}, \bar\partial_{\dot{B}}\} =0.\]
However, notice that by (\ref{eq:superspace_diff_product_rule})
\[ \{\partial_A, \theta_B\}=\partial_A\theta_B- \theta_B\partial_A +\theta_B\partial_A =  \delta_{AB},\]
and similar relations. We can show that the indices of these differential operators can be raised and lowered with the $\epsilon$ in Eqs.~(\ref{eq:epsilonAB}) and (\ref{eq:epsilonAdotBdot}).

\df{The integral operator of a function $f$ of a superspace coordinate $\theta_A$ is defined as a functional $I[f]$
...
and similarly for $\bar\theta_{\dot A}$. We demand linearity:
\[\int d\theta_A[af(\theta_A) + bg(\theta_A)] \equiv a\int d\theta_A f(\theta_A) + b\int d\theta_A g(\theta_A),\]
and that the integral operators anti-commute, just as the differential operators
...
\bar{D}_{\dot{A}} &\equiv& -\bar\partial_{\dot{A}}-i(\theta\sigma^\mu)_{\dot{A}}\partial_\mu.
\end{eqnarray}}
...
\df{A {\bf superfield} $\Phi$ is a function on superspace $\Phi(x, \theta, \bar{\theta})$.}
...
\delta_S\psi_A (y) &=& \sqrt{2}\alpha_A F(y) +\frac{i}{\sqrt{2}} (\sigma^\mu\bar\alpha)_A\partial_\mu A(y), \\
...
\begin{Exercise}[label=left_handed_scalar_superield_x]
...
\begin{Answer}
The goal here is to find the component field transformations for a vector superfield $V$ under a supergauge transformation
\begin{equation*}
    V \to V' = V - i [\Lambda - \Lambda^\dagger],
\end{equation*}
where $\Lambda$ is a scalar superfield. The general form for a vector superfield is,
\begin{equation}
    \begin{aligned}
        V(x, \theta, \bar{\theta}) &= C(x) + \theta^{A}\chi_A(x) + \bar{\theta}_{\dot{A}}\bar{\chi}^{\dot{A}}(x) + \theta \theta M(x) + \bar{\theta}\bar{\theta}M^*(x) \\
...
\end{equation}
and as we saw in Ex.~\ref{left_handed_scalar_superield_x} we can write a scalar superfield as
\begin{align}
    \label{venstrehendt skalarfelt2} 
    \Phi & =A(x) + i\theta\sigma^\mu\bar{\theta}\partial_\mu A(x) - \frac{1}{4}\theta\theta\bar{\theta}\bar{\theta}\Box A(x) + \sqrt{2}\theta\psi(x) - \frac{i}{\sqrt{2}}\theta\theta (\sigma^\mu\bar\theta)^A\partial_\mu \psi_A (x) + \theta \theta F(x),\\
    \label{høyrehendt skalarfelt2}
    \Phi^\dagger &= A^*(x) - i\theta \sigma^{\mu}\bar{\theta}\partial_\mu A^*(x) - \frac{1}{4}\theta\theta\bar{\theta}\bar{\theta}\Box A^*(x) + \sqrt{2}\bar{\theta}\bar{\psi}(x) + \frac{i}{\sqrt{2}}\bar{\theta} \bar{\theta}(\theta\sigma^{\mu})_{\dot{A}}\partial_\mu\bar{\psi}^{\dot{A}}(x) + \bar{\theta}\bar{\theta}F^*(x).
\end{align}

Using $\Lambda = i \Phi$, and Eqs.~\eqref{vektorsuperfelt2}--\eqref{høyrehendt skalarfelt2}, we get
\begin{align}
    V \to V' &= V - i [\Lambda - \Lambda^\dagger] = V - i\Lambda - (i\Lambda)^\dagger \nonumber\\
   ...
   C + \theta^{A}\chi_A + \bar{\theta}_{\dot{A}}\bar{\chi}^{\dot{A}} + \theta \theta M + \bar{\theta}\bar{\theta}M^* + (\theta \sigma^\mu \bar{\theta})V_{\mu} + \theta \theta \bar{\theta}_{\dot{A}}\bar{\lambda}^{\dot{A}} + \bar{\theta}\bar{\theta}\theta^A\lambda_{A} + \theta\theta \bar{\theta}\bar{\theta}D \nonumber \\
    [\eqref{venstrehendt skalarfelt2}] \quad \quad     & \quad \, +A + i(\theta\sigma^\mu\bar{\theta})\partial_\mu A - \frac{1}{4}\theta\theta\bar{\theta}\bar{\theta}\Box A + \sqrt{2}\theta^A\psi_{A} - \frac{i}{\sqrt{2}}\theta\theta (\sigma^\mu\bar\theta)^A\partial_\mu \psi_A + \theta \theta F \nonumber \\
    [\eqref{høyrehendt skalarfelt2}] \quad \quad    & \quad \, +A^* - i(\theta \sigma^{\mu}\bar{\theta})\partial_\mu A^* - \frac{1}{4}\theta\theta\bar{\theta}\bar{\theta}\Box A^* + \sqrt{2}\bar{\theta}_{\dot{A}}\bar{\psi}^{\dot{A}} + \frac{i}{\sqrt{2}}\bar{\theta} \bar{\theta}(\theta\sigma^{\mu})_{\dot{A}}\partial_\mu\bar{\psi}^{\dot{A}} + \bar{\theta}\bar{\theta}F^* \nonumber \\
    &= C + A^* + A + \theta^A( \chi_A + \sqrt{2}\psi_A) + \bar{\theta}_{\dot{A}}(\bar{\chi}^{\dot{A}} + \sqrt{2}\bar{\psi}^{\dot{A}}) + \theta \theta (M + F) + \bar{\theta}\bar{\theta}(M^* + F^*) \nonumber \\
    &\quad \, + (\theta \sigma^\mu \bar{\theta})[V_\mu + i \partial_\mu(A - A^*)] \nonumber\\
    &\quad\,  +\theta \theta \bar{\theta}_{\dot{A}}\left( \bar{\lambda}^{\dot{A}} - \frac{i}{\sqrt{2}}\sigma^{\mu B}_{\:\:\,\dot{A}}\partial_\mu \psi_B\right) + \bar{\theta}\bar{\theta}\theta^{A}\left(\lambda_A + \frac{i}{\sqrt{2}}\sigma^{\mu}_{A\dot{B}}\partial_\mu\bar{\psi}^{\dot{B}}\right) \nonumber \\
    & \quad \, + \theta \theta \bar{\theta}\bar{\theta}\left(D -  \frac{1}{4}\Box[A + A^*]\right) 
...
\end{align}
In the final two equalities we have the transformed component fields and we can compare to the original expression in \eqref{vektorsuperfelt2} to read off how the component fields  transform under a supergauge transformation. We notice that  $\Box[A + A^*]$ makes an appearance together with the $D$-field, and that  $[A + A^*]$ appears with the $C$-field. By redefining $D \to D - \frac{1}{4}\Box C$, the last term of the supergauge transformation has the form 
\begin{align*}
    \theta \theta \bar{\theta}\bar{\theta}D' = \theta \theta \bar{\theta}\bar{\theta}\left(D -  \frac{1}{4}\Box[A + A^*]\right) \xrightarrow[D \to D - \frac{1}{4}\Box C]{}\, & \theta \theta \bar{\theta}\bar{\theta}\left(D - \frac{1}{4}\Box C - \frac{1}{4}\Box[A + A^*]\right) \\
    =\, & \theta \theta \bar{\theta}\bar{\theta}\left(D -  \frac{1}{4}\Box[C + A + A^*]\right) = \theta \theta \bar{\theta}\bar{\theta}(D' + C').
\end{align*}
We can do the same with $\lambda$. Let $\lambda_A \to \lambda_A + \frac{i}{2}\sigma^\mu_{A\dot{B}}\partial_\mu \bar{\chi}^{\dot{B}}$. This is motivated by $\chi_A' = \chi_A + \sqrt{2}\psi_A$. We do the calculation for $\lambda$, but the same holds for  $\bar{\lambda} = \lambda^\dagger$:
\begin{align*}
    \bar{\theta}\bar{\theta}\theta^{A}\lambda_A' = \bar{\theta}\bar{\theta}\theta^{A}\left(\lambda_A + \frac{i}{\sqrt{2}}\sigma^\mu_{A\dot{B}}\partial_\mu \bar{\psi}^{\dot{B}}\right) \xrightarrow[\lambda_A \to \lambda_A + \tfrac{i}{2}\sigma^\mu_{A\dot{B}}\bar{\chi}^{\dot{B}}]{} 
...
\end{align*}
With these two redefinitions we see that neither $D$ nor $\lambda$ transform under the supergauge transformation. The other fields can be read directly from Eq.~\eqref{superjustér-vektortrafo}. In all we have
\begin{align*}
    C &\to C' = C + A + A^*, \\
    \chi_A &\to \chi_A' = \chi_A + \sqrt{2}\psi_A, \\
    M & \to M' = M + F, \\
    V_\mu &\to V_\mu' = V_\mu + i \partial_\mu(A - A^*),\\
    \lambda_A &\to \lambda_A' = \lambda_A,   \quad \quad [\text{no transformation}] \\
    D &\to D' = D. \quad \quad \, \,\,[\text{no transformation}]
\end{align*}
\end{Answer}
%%%
