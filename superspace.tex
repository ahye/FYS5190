...
because one can show that\footnote{Fortunately we are not going to do this because it is messy, but it can be done using the algebra of the group and the series expansion of the exponential function. Note, however, that the proof rests on the $P$s and $Q$s forming a closed set, which we saw in the algebra Eqs.~(\ref{eq:QQweyl})--(\ref{eq:QMweyl}).}
\begin{equation}
...
\begin{Exercise}
Find the expression for $V_{WZ}^2$.
\end{Exercise}
