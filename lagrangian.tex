...
In Sec.~\ref{sec:superalgebra} we  saw that the only possible nontrivial interaction between a gauge group and the supersymmetry generators for $N=1$ supersymmetry is given by  (\ref{eq:QR_commutator_dirac}), which in Weyl spinor notation for the supercharges is
\begin{equation}
...
\begin{Exercise}[]
\label{ex:simpleL}
Write down the Lagrangian and find the action of the simplest possible (renormalisable) supersymmetric field theory with a single scalar superfield  $\Phi$, without gauge transformations, in terms of component fields, and show that it contains no kinetic terms for the $F$-field. Then show how the $F$-field can be eliminated by the equations of motion. {\it Hint:} The kinetic part of the action will turn out to be
\begin{equation*}
...
\begin{Answer}
The most general Lagrangian for a single scalar superfield $\Phi$ is
\begin{equation*}
...
+i\bar{\theta}\bar{\theta}\theta \sigma^\mu\partial_\mu \bar{\psi}\theta \psi - i\bar{\theta}\bar{\psi}\theta\theta\partial_\mu \psi\sigma^\mu\bar{\theta}\right\},\nonumber
\end{eqnarray}
where we have removed the factor $\theta\theta\bar{\theta}\bar{\theta}$ by integration in the first three terms. Using the identities
\begin{eqnarray}
...
gives
\begin{eqnarray}
\int d^4\theta \,\Phi^\dagger\Phi&=& - \frac{1}{4}A\Box A^* -\frac{1}{4} A^*\Box A +\frac{1}{2}\partial^\mu A\partial_\mu A^* +  |F|^2 -\frac{i}{2}\psi\sigma^\mu\partial_\mu \bar\psi+ \frac{i}{2}\partial_\mu\psi\sigma^\mu \bar\psi.\nonumber
\end{eqnarray}
Now, since
\begin{equation}
