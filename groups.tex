...
\begin{itemize}
\item[i)] $\exp(M^T)=\exp(M)^T$
\item[ii)] $\exp(M^*)=\exp(M)^*$
\item[iii)] If $A$ is an invertible matrix then $\exp(AMA^{-1})=A\exp(M)A^{-1}$
\item[iv)] If $[M,N]=0$ then $\exp(M)\exp(N)=\exp(M+N)$
\end{itemize}
...
At the end, let us make a warning: despite the enticing notation it is not in general the case that if $H$ is a normal subgroup to $G$, then $G\cong G/H\times H$.
...
In physics an important category of representations are the {\bf unitary representations}, which are linear?representations $\rho$ of a group $G$ on a complex Hilbert space $V$ where $\rho(g)$ is a unitary operator, {\it i.e.} an operator that preserves the length of vectors in $V$, for every $g\in G$.

This generalises the unitary (matrix) group, which are examples of unitary representations, beyond finite dimensional vector spaces.
...
If  we also have $\rho_{12}(g)=0$ in our example we say that the representation is {\bf completely reducible}. It can be shown that in most cases a reducible representation is also completely reducible. In fact, representations for which this is not true tend to be mathematical curiosities. For example, if the vector space is a Hilbert space (states in quantum mechanics), there exist a proof that all representations are completely reducible.  As a result, there is a tendency in physics to use the term ``reducible" where we should use the term ``completely reducible". In the case of a completely reducible representation we can split the vector space $V$ into a direct sum of two vector spaces $V=W_1\oplus W_2$, where $\mathbf v_1\in W_1$ and $\mathbf v_2\in W_2$, and define a representation of $G$ on each of them using $\rho_1$ and $\rho_2$, which in turn could either be reduced more, or would be irreducible.
...
\theo{(Schur's Lemma~\cite{Schur})\\ If we have an irreducible representation $\rho$ of a group $G$, all the matrices $A$ that commute with $\rho(g)$ for all $g \in G$ are proportional to the identity, $A=\lambda I$. Here the $\lambda$ are constants that label the representation.}
