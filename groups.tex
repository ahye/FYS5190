...
In Sec.~\ref{sec:generators} we saw the two sets of generators of the Lie algebras $\mathfrak{su(2)}$ and $\mathfrak{so(3)}$, derived from the defining matrix representations of the groups. Since this is the same algebra this shows two distinct representations of the same algebra. 
...
For $SU(2)$ the generators taken from the fundamental representation $\mathbf 2$ were the Pauli matrices $\sigma_i$, and the exponential map is thus, as we alluded to earlier, $e^{i\alpha_i\sigma^i}$, where $\alpha_i$ are the parameters. Beyond the fundamental representations $SU(2)$ has irreducible representations in all dimensions $\mathbf 2$, $\mathbf 3$, $\mathbf 4$, $\mathbf 5$, \ldots.
...
To see why this makes sense we need to look at the exponential map of the dual representation. If $X_i$ is a generator in the representation $\rho$, then the dual representation $\rho^*$ tells us to use instead $-X_i^T$. In the exponentiation the group element $g=\exp(iX_i)$ becomes the group element $\exp(-iX_i^T)=\exp(-iX_i)^T=(g^{-1})^T$ with the dual representation.
