...
\end{equation}
You may recognise these as related to the angular momentum operators in quantum mechanics. As we will see later, these matrices will form the basis for creating the three-dimensional adjoint representation of $SU(2)$, named {\bf 3}. In the Standard Model this is the representation used for operations on the gauge fields that transform under $SU(2)_L$, which are represented as three-component vectors. The central point here is that the group structure is the same (isometric), even if the objects in the representation are different.
...
\end{eqnarray*}
This is the result of the transformation by the member of the group that in the parameterisation sits $d\mathbf a$ from the identity. For a matrix group where the matrix $A$ is parametrised as $A(\mathbf a)$ the resulting change in the vector space is
\[
dx_i=\left.da_j\frac{\partial f_i}{\partial a_j}\right|_{\mathbf a =0}=da_j\left[\frac{\partial A(\mathbf a)}{\partial a_j}\mathbf{x}\right]_{i,\mathbf a=0}=[da_jX^j\mathbf{x}]_i,
\]
where we have defined 
\[
X^j\equiv \left. \frac{\partial A(\mathbf a)}{\partial a_j}\right|_{\mathbf a=0} ,
\]
...
Notice that we can show the following relationship for these two generators: $[X^1, X^2] \equiv X^1X^2-X^2X^1= iX^2$.
...
A similar calculation for $SO(3)$, which also has three free parameters, gives the generators $X^i=\frac{1}{2}L^i$, where $L^i$ are the three matrices in Eq~(\ref{eq:SO3_generators}). These generators have the exact same commutator, $[X_i,X_j]=i\epsilon_{ijk}X^k$, indicating a structural similarity between the groups $SU(2)$ and $SO(3)$. For $SO(3)$ we can further find a corresponding set of differential generators
\[
...
In general, the commutator of the generators of a Lie group satisfy \[[X_i,X_j]=iC_{ij}^{~~k}X_k,\] where  $C_{ij}^{~~k}$ are called the {\bf structure constants} of the group.\footnote{There is an annoying difference in notation here between physics and mathematics, where the $i$ is commonly dropped.} 
We can easily see that  these are antisymmetric in the indices $i$ and $j$, $C_{ij}^{~~k} = -C_{ji}^{~~k}$. Lie also showed~\cite{MR1510035} that there is a {\bf Jacobi identity} among the generators,
\begin{equation}
[X_i, [X_j, X_k]] + [X_j, [X_k, X_i]] + [X_k, [X_i, X_j]] = 0.
\end{equation}
This immediately leads to a corresponding identity for the structure constants:
\begin{equation}
...
We discussed the defining or fundamental representation of a matrix group earlier. These representations usually have the lowest possible dimension. The definition of the structure constants allows us to also introduce the {\bf adjoint representation}, which consists of the $n$ matrices $M_i$ with elements:
\[(M_i)_j^{~k} = -iC_{ij}^{~~k},\]
where $C_{ij}^{~~k},$ are the structure constants. From the Jacobi identity it follows that
\[ [M_i, M_j] = iC^{~~k}_{ij}M_k,\]
meaning that the matrices of the adjoint representation fulfils the same commutation relationship as the fundamental (generators). Note that the dimension of the adjoint representation for $SO(m)$ and $SU(m)$ is equal to the number of independent parameters, $m(m-1)/2$ and $m^2-1$, respectively.

We can now briefly return to the $SU(2)$ and $SO(3)$ examples. These groups have the same structure constants because they have the same commutators between generators. Thus their adjoint representations are the same. With structure constants $C^{~~k}_{ij}=\epsilon_{ij}^{~~k}$ we get the elements of the adjoint matrices $(M_i)_{jk} =-i\epsilon_{ijk}$, which gives $M_i=iL_i$, with the $L_i$ being the matrices in (\ref{eq:SO3_generators}). 
